<?php
/*
Plugin Name: Advance your Editor
Description: Advance your Classic editor and Block editor (Gutenberg) with new functionalities like tables, font, search and replace, etc.
Author: reevs02
Version: 2.0.0
License: GPL2
License URI: https://www.gnu.org/licenses/gpl-2.0.html
Text Domain: advance_your_editor
*/
...
if ( ! class_exists( 'Advance_Your_Editor' ) ) :

class Advance_Your_Editor {
	...
	public function load_textdomain() {
	    load_plugin_textdomain( 'advance_your_editor', false, 'advance_your_editor/langs' );
	}

	public function enqueue_scripts( $page ) {
		if ( 'settings_page_advance_your_editor' === $page ) {
	...
	public function warn_if_unsupported() {
		if ( ! $this->check_minimum_supported_version() ) {
			...
			printf(
				__( 'Advance your Editor requires WordPress version %1$s or newer. It appears that you are running %2$s. This can make the editor unstable.', 'advance_your_editor' ),
			...
			printf(
				__( 'Please upgrade your WordPress installation or download an <a href="%s">older version of the plugin</a>.', 'advance_your_editor' ),
				'https://wordpress.org/plugins/advance_your_editor/advanced/#download-previous-link'
			);
	...
	public function block_editor_assets() {
		...
		if ( $this->check_admin_setting( 'hybrid_mode' ) || $this->check_admin_setting( 'classic_paragraph_block' ) ) {
			...
			if ( $this->check_admin_setting( 'classic_paragraph_block' ) ) {
				$strings = array(
					'classicParagraphTitle' => __( 'Classic Paragraph', 'advance_your_editor' ),
					'classicParagraph' => 'yes',
					'description' => __( 'For use instead of the Paragraph Block. Supports transforming to and from multiple Paragraph blocks, Image, Table, List, Quote, Custom HTML, and most other blocks.', 'advance_your_editor' ),
				);
		...
		if ( ! empty( $this->toolbar_block ) || ! empty( $this->toolbar_block_side ) || ! empty( $this->panels_block ) ) {
			...
			$strings = array(
				...
				// Strings
				'strFormatting' => __( 'Formatting', 'advance_your_editor' ),
				'strRemoveFormatting' => __( 'Clear formatting', 'advance_your_editor' ),
				'strSuperscript' => __( 'Superscript', 'advance_your_editor' ),
				'strSubscript' => __( 'Subscript', 'advance_your_editor' ),
				'strMark' => __( 'Mark', 'advance_your_editor' ),
				'strUnderline' => __( 'Underline', 'advance_your_editor' ),

				'strTextColor' => __( 'Text Color', 'advance_your_editor' ),
				'strTextColorLabel' => __( 'Selected text color', 'advance_your_editor' ),
				'strBackgroundColor' => __( 'Text Background Color', 'advance_your_editor' ),
				'strBackgroundColorLabel' => __( 'Selected text background color', 'advance_your_editor' ),
			);
	...
	public function import_export_settings_file() {
		...
		if (
			...
			$url = admin_url( 'options-general.php?page=advance_your_editor' );
	...
	public function add_menu() {
		add_options_page( 'Advance your Editor', 'Advance your Editor', 'manage_options', 'advance_your_editor', array( $this, 'settings_page' ) );
	}
	...
	public function add_settings_link( $links, $file ) {
		if (
			strpos( $file, '/advance_your_editor.php' ) !== false &&
			plugin_basename( __FILE__ ) === $file &&
			current_user_can( 'manage_options' )
		) {
			$settings_link = sprintf( '<a href="%s">%s</a>', admin_url( 'options-general.php?page=advance_your_editor' ), __( 'Settings', 'advance_your_editor' ) );
			$links = (array) $links;
