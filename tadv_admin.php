...
if ( isset( $_POST['tadv-save'] ) ) {
	...
	$message = '<div class="updated notice notice-success is-dismissible"><p>' .  __( 'Default settings restored.', 'advance_your_editor' ) . '</p></div>';
} elseif ( isset( $_POST['tadv-import-settings'] ) ) {
	check_admin_referer( 'tadv-save-buttons-order' );

	// TODO: all users
	?>
	<div class="wrap advance_your_editor">
	<h2><?php _e( 'Advance your Editor Settings Import', 'advance_your_editor' ); ?></h2>

	<div class="tadv-import-export">
		<form action="" method="post" enctype="multipart/form-data" class="import-file">
			<p><?php _e( 'The settings are imported from a previously exported settings file.', 'advance_your_editor' ); ?></p>
			<p><input type="file" name="tadv-import"></p>
			<p><input type="submit" class="button button-primary" name="tadv-import-file" value="<?php _e( 'Import settings', 'advance_your_editor' ); ?>" /></p>
			<?php wp_nonce_field( 'tadv-import-settings', 'tadv-import-settings-nonce', false ); ?>
		</form>
		<hr>
		<form action="<?php echo $form_action; ?>" method="post">
			<p><?php _e( 'Alternatively the settings can be imported from a JSON encoded string. Please paste the exported string in the text area below.', 'advance_your_editor' );	?></p>
			<p><textarea id="tadv-import" name="tadv-import"></textarea></p>
			<p>
				<button type="button" class="button" id="tadv-import-verify"><?php _e( 'Verify', 'advance_your_editor' ); ?></button>
				<input type="submit" class="button button-primary alignright" name="tadv-import-submit" value="<?php _e( 'Import settings from string', 'advance_your_editor' ); ?>" />
			</p>
			<?php wp_nonce_field( 'tadv-import-settings', 'tadv-import-settings-nonce', false ); ?>
			<p id="tadv-import-error"></p>
		</form>
		<p><a href=""><?php _e( 'Back to Editor Settings', 'advance_your_editor' ); ?></a></p>
	</div>
	</div>
	<?php
	...
	if ( ! is_array( $import ) ) {
		$message = '<div class="error notice is-dismissible"><p>' .  __( 'Importing of settings failed.', 'advance_your_editor' ) . '</p></div>';
	} else {
		$this->save_settings( $import );
		$message = '<div class="updated notice notice-success is-dismissible"><p>' . __( 'Settings imported successfully.', 'advance_your_editor' ) . '</p></div>';
	}
	...
	switch( $err ) {
		case 1:
			$message = __( 'Importing of settings failed. Please import a valid settings file.', 'advance_your_editor' );
			break;
		case 2:
			$message = __( 'Importing of settings failed. The imported file is empty.', 'advance_your_editor' );
			break;
		case 3:
			$message = __( 'Importing of settings failed. The imported file is invalid.', 'advance_your_editor' );
			break;
	}
...
if ( empty( $this->toolbar_1 ) && empty( $this->toolbar_2 ) && empty( $this->toolbar_3 ) && empty( $this->toolbar_4 ) ) {
	$message = '<div class="error"><p>' . __( 'ERROR: All toolbars are empty. Default settings loaded.', 'advance_your_editor' ) . '</p></div>';
...
?>
<div class="wrap advance_your_editor block-active<?php if ( is_rtl() ) echo ' mce-rtl'; ?>" id="contain">
<h2><?php _e( 'Editor Settings', 'advance_your_editor' ); ?></h2>
<?php
...
if ( isset( $_POST['tadv-save'] ) && empty( $message ) ) {
	?><div class="updated notice notice-success is-dismissible"><p><?php _e( 'Settings saved.', 'advance_your_editor' ); ?></p></div><?php
} else {
	echo $message;
}
...
<div class="toggle">
	<p class="tadv-submit tadv-submit-top">
		<input class="button-primary button-large top-button" type="submit" name="tadv-save" value="<?php _e( 'Save Changes', 'advance_your_editor' ); ?>" />
	</p>
	<h3 class="settings-toggle block" tabindex="0">
		<span class="dashicons dashicons-arrow-down"></span>
		<span class="dashicons arrow-open <?php echo $dashicons_arrow; ?>"></span>
		<?php _e( 'Block Editor (Gutenberg)', 'advance_your_editor' ); ?>
	</h3>
	<h3 class="settings-toggle classic" tabindex="0">
		<span class="dashicons dashicons-arrow-down"></span>
		<span class="dashicons arrow-open <?php echo $dashicons_arrow; ?>"></span>
		<?php _e( 'Classic Editor', 'advance_your_editor' ); ?>
	</h3>
</div>

<div id="block-editor">
<h4><?php _e( 'Toolbars for the Block Editor', 'advance_your_editor' ); ?></h4>
...
?>
	...
	<div>
		<p class="toolbar-block-title">
			<strong><?php _e( 'Main toolbar', 'advance_your_editor' ); ?></strong>
			<br>
			<span class="tma-help tadv-popout-help-toggle">
				<span class="dashicons dashicons-editor-help"></span>
				<?php _e( 'Limitations for the Block Editor toolbar', 'advance_your_editor' ); ?>
			</span>
		</p>
		...
		<div class="tadv-popout-help hidden">
			...
			<ol>
				<li><?php _e( 'The Align Left, Align Center, Align Right, Bold, Italic, and Link buttons cannot be moved or arranged.', 'advance_your_editor' ); ?></li>
				<li><?php _e( 'All other buttons are always shown in a drop-down. The users are not allowed to add any of them to the main toolbar.', 'advance_your_editor' ); ?></li>
				<li><?php _e( 'All buttons that are shown in the drop-down are auto-arranged by alphabetical order. The users are not allowed to arrange them.', 'advance_your_editor' ); ?></li>
				<li><?php _e( 'The drop-down cannot be empty and the Inline Image and Text Color items cannot be placed in the side toolbar.', 'advance_your_editor' ); ?>
			</ol>
		</div>
		...
		<p class="toolbar-block-title">
			<strong><?php _e( 'Alternative side toolbar', 'advance_your_editor' ); ?></strong>
			<?php _e( '(shown in the sidebar)', 'advance_your_editor' ); ?>
		</p>
		<div class="toolbar-side-wrap toolbar-wrap">
			<div class="panel-title">
				<?php _e( 'Formatting', 'advance_your_editor' ); ?>
		...
		<p class="toolbar-block-title">
			<strong><?php _e( 'Unused buttons for the blocks toolbars', 'advance_your_editor' ); ?></strong>
		</p>
		...
		<div class="panel-block-colors-wrap">
			<div class="panel-block-colors">
				...
				<div class="panel-block-background-color<?php if ( ! $this->check_user_setting( 'selected_text_background_color' ) ) echo ' disabled'; ?>">
					<p><?php _e( 'Selected text background color', 'advance_your_editor' ); ?></p>
					<img width="260" height="100" class="text-color-preview" src="<?php echo $colors_preview_src; ?>">
				</div>
			</div>
			...
			<table class="form-table panel-block-colors-settings"><tbody>
				<tr class="panel-block-colors-settings__text">
					<th>
						<?php _e( 'Enable setting of selected text color', 'advance_your_editor' ); ?>
						<span class="small-info"><?php _e( '(this will replace the default text color popup in WP 5.4 and newer)', 'advance_your_editor' ); ?></span>
					</th>
					<td>
						<p>
							...
							<label for="selected_text_color_yes"><?php _e( 'Yes', 'advance_your_editor' ); ?></label>
						</p>
						<p>
							...
							<label for="selected_text_color_no"><?php _e( 'No', 'advance_your_editor' ); ?></label>
						</p>
					</td>
				</tr>
				<tr class="panel-block-colors-settings__background">
					<th><?php _e( 'Enable setting of selected text background color', 'advance_your_editor' ); ?></th>
					<td>
						<p>
							...
							<label for="selected_text_background_color_yes"><?php _e( 'Yes', 'advance_your_editor' ); ?></label>
						</p>
						<p>
							...
							<label for="selected_text_background_color_no"><?php _e( 'No', 'advance_your_editor' ); ?></label>
						</p>
					</td>
				</tr>
			</tbody></table>
...
<h4 class="classic-blocks-title-h4"><?php _e( 'Toolbars for the Classic Paragraph and Classic blocks', 'advance_your_editor' ); ?></h4>

<p>
	<?php _e( 'The toolbars in the Classic Paragraph and Classic blocks are narrower and show on focus.', 'advance_your_editor' ); ?>
	<?php _e( 'For best results enable the menu and add only essential buttons.', 'advance_your_editor' ); ?>
	<?php _e( 'The buttons will wrap around depending on the width of the toolbar.', 'advance_your_editor' ); ?>
</p>
...
<p>
	...
	<label for="menubar_block"><?php _e( 'Enable the editor menu (recommended).', 'advance_your_editor' ); ?></label>
</p>
...
<p><?php _e( 'Drop buttons in the toolbars, or drag the buttons to rearrange them.', 'advance_your_editor' ); ?></p>

<div class="unuseddiv">
	<p><strong><?php _e( 'Unused Buttons for the Classic Paragraph and Classic blocks toolbars', 'advance_your_editor' ); ?></strong></p>
	<div>
...
<h4><?php _e( 'Toolbars for the Classic Editor', 'advance_your_editor' ); ?></h4>

<div class="tadvzones">
<p>
	<input type="checkbox" name="options[]" id="menubar" value="menubar" <?php if ( $this->check_user_setting( 'menubar' ) ) { echo ' checked="checked"'; } ?>>
	<label for="menubar"><?php _e( 'Enable the editor menu.', 'advance_your_editor' ); ?></label>
</p>
...
<div class="unuseddiv">
	<h4><?php _e( 'Unused Buttons', 'advance_your_editor' ); ?></h4>
	<div>
...
<div class="advanced-options">
	<h3><?php _e( 'Options', 'advance_your_editor' ); ?></h3>
	...
	<div>
		...
		<label for="merge_toolbars"><?php _e( 'Append all buttons to the top toolbar in the Classic Paragraph and Classic blocks.', 'advance_your_editor' ); ?></label>
		<p><?php _e( 'This affects buttons that are added by other plugins. These buttons will be appended to the top toolbar row instead of forming second, third, and forth rows.', 'advance_your_editor' ); ?></p>
	</div>

	<div>
		<input type="checkbox" name="options[]" value="advlist" id="advlist" <?php if ( $this->check_user_setting('advlist') ) echo ' checked'; ?> />
		<label for="advlist"><?php _e( 'List Style Options', 'advance_your_editor' ); ?></label>
		<p>
			<?php _e( 'Enable more list options: upper or lower case letters for ordered lists, disk or square for unordered lists, etc.', 'advance_your_editor' ); ?>
		</p>
	</div>
	...
	<div>
		...
		<label for="contextmenu"><?php _e( 'Context Menu', 'advance_your_editor' ); ?></label>
		<p><?php _e( 'Replace the browser context (right-click) menu.', 'advance_your_editor' ); ?></p>
	</div>

	<div>
		<input type="checkbox" name="options[]" value="advlink" id="advlink" <?php if ( $this->check_user_setting('advlink') ) echo ' checked'; ?> />
		<label for="advlink"><?php _e( 'Alternative link dialog', 'advance_your_editor' ); ?></label>
		<p><?php _e( 'Open the link dialog when using the link button on the toolbar or the link menu item.', 'advance_your_editor' ); ?></p>
	</div>
	...
	<div>
		...
		<label for="fontsize_formats"><?php _e( 'Font sizes', 'advance_your_editor' ); ?></label>
		<p><?php printf( __( 'Replace the size setting available for fonts with: %s.', 'advance_your_editor' ), $this->fontsize_formats ); ?></p>
	</div>
</div>
<?php
...
if ( ! is_multisite() || current_user_can( 'manage_sites' ) ) {
	?>
	<div class="advanced-options">
	<h3><?php _e( 'Advanced Options', 'advance_your_editor' ); ?></h3>
	<div>
		...
		<label for="classic_paragraph_block"><?php _e( 'Add Classic Paragraph block', 'advance_your_editor' ); ?></label>
		<p>
			<?php _e( 'The Classic Paragraph block includes the familiar editor and is an extended and enhanced Classic block.', 'advance_your_editor' ); ?>
			<?php _e( 'You can add multiple paragraphs, tables, galleries, embed video, set fonts and colors, and generally use everything that is available in the Classic Editor.', 'advance_your_editor' ); ?>
			<?php _e( 'Also, like the Classic block, most existing plugins and add-ons will continue to work.', 'advance_your_editor' ); ?>
			<?php _e( 'This makes the Block Editor more familiar, easier to use, easier to get used to, and more compatible with your existing workflow.', 'advance_your_editor' ); ?>
		</p>
		<p>
			<?php _e( 'In addition most default blocks can be transformed into classic paragraphs, and a Classic Paragraph can be converted to multiple blocks.', 'advance_your_editor' ); ?>
			<?php _e( 'It can be used everywhere instead of the Paragraph block including in columns, when creating reusable blocks, etc.', 'advance_your_editor' ); ?>
		</p>
	</div>
	...
	<div>
		...
		<label for="hybrid_mode"><?php _e( 'Make the Classic Paragraph or Classic block the default block (hybrid mode)', 'advance_your_editor' ); ?></label>
		<p>
			<?php _e( 'The default block is inserted on pressing Enter in the title, or clicking under the last block.', 'advance_your_editor' ); ?>
			<?php _e( 'Selecting this option also adds some improvements and fixes for the Classic block.', 'advance_your_editor' ); ?>
		</p>
	</div>
	...
	<div>
		...
		if ( function_exists( 'is_plugin_active' ) && ! is_plugin_active( 'classic-editor/classic-editor.php' ) ) {
			...
			<label for="replace_block_editor"><?php _e( 'Replace the Block Editor with the Classic Editor', 'advance_your_editor' ); ?></label>
			<p>
				<?php _e( 'Selecting this option will restore the previous (&#8220;classic&#8221;) editor and the previous Edit Post screen.', 'advance_your_editor' ); ?>
				<?php _e( 'It will allow you to use other plugins that enhance that editor, add old-style Meta Boxes, or in some way depend on the previous Edit Post screen.', 'advance_your_editor' ); ?>
			</p>
			<p>
				<?php

				$text = __( 'If you prefer to use both editors side by side, do not enable this option. It is better to install the %1$sClassic Editor plugin%2$s.', 'advance_your_editor' );
				/* translators: URL to (localised) Classic Editor plugin. */
				$url = __( 'https://wordpress.org/plugins/classic-editor/', 'advance_your_editor' );
	...
	<div>
		...
		<label for="no_autop"><?php _e( 'Keep paragraph tags in the Classic block and the Classic Editor', 'advance_your_editor' ); ?></label>
		<p>
			<?php _e( 'Stop removing &lt;p&gt; and &lt;br&gt; tags in the Classic Editor and show them in the Text tab.', 'advance_your_editor' ); ?>
			<?php _e( 'This will make it possible to use more advanced coding in the Text tab without the back-end filtering affecting it much.', 'advance_your_editor' ); ?>
			<?php _e( 'However it may behave unexpectedly in rare cases, so test it thoroughly before enabling it permanently.', 'advance_your_editor' ); ?>
			<?php _e( 'Line breaks in the Text tab in the Classic Editor would still affect the output, in particular do not use empty lines, line breaks inside HTML tags or multiple &lt;br&gt; tags.', 'advance_your_editor' ); ?>
		</p>
	</div>
	<?php
	...
	<div>
		...
		<label for="importcss"><?php _e( 'Create CSS classes menu', 'advance_your_editor' ); ?></label>
		<p>
			<?php _e( 'Load the CSS classes used in editor-style.css and replace the Formats menu.', 'advance_your_editor' ); ?>
		</p>
		<?php
		...
		if ( $disabled ) {
			?>
			<p>
			<span class="tadv-error"><?php _e( 'Disabled:', 'advance_your_editor' ); ?></span>
			<?php _e( 'A stylesheet file named editor-style.css was not added by your theme.', 'advance_your_editor' ); ?>
			<br>
			<?php
		}
	...
	<div class="advanced-table-options">
		<h4><?php _e( 'Advanced options for tables', 'advance_your_editor' ); ?></h4>
		<div>
			...
			<label for="table_resize_bars"><?php _e( 'Enable resizing of tables, rows, and columns by dragging with the mouse', 'advance_your_editor' ); ?></label>
			<p>
				<?php _e( 'When enabled the whole table, rows, and columns can be resized by dragging but the sizes are set with inline CSS styles.', 'advance_your_editor' ); ?>
				<?php _e( 'This may override some styles that are set by your theme and usually makes the table non-responsive when viewed on a small screen like a smartphone.', 'advance_your_editor' ); ?>
				<?php _e( 'When a row or a column is resized the inline styles are updated on all table rows and cells.', 'advance_your_editor' ); ?>
			</p>
			<p>
				<?php _e( 'Disabling this option will stop the editor from adding inline CSS styles and will produce cleaner HTML code.', 'advance_your_editor' ); ?>
				<?php _e( 'Then the table, the rows and the cells can be resized by typing the size values in the advanced options tabs.', 'advance_your_editor' ); ?>
			</p>
			<p>
				<span class="dashicons dashicons-info"></span>
				<?php _e( 'This option does not affect inline styles on tables in existing posts. To reset table size or remove all formatting for the whole table please see the two buttons at the bottom of the Format menu.', 'advance_your_editor' ); ?>
			</p>
		</div>
		<div>
			...
			<label for="table_default_attributes"><?php _e( 'When inserting a table set the HTML border attribute to 1', 'advance_your_editor' ); ?></label>
			<p>
				<?php _e( 'This will add a border around the table unless it is overriden by your theme.', 'advance_your_editor' ); ?>
				<?php _e( 'To set other default attributes or inline styles use the Advanced Configuration plugin.', 'advance_your_editor' ); ?>
			</p>
		</div>
		<div>
			...
			<label for="table_grid"><?php _e( 'When inserting a table show a grid where the number of rows and columns can be selected by dragging with the mouse', 'advance_your_editor' ); ?></label>
			<p>
				<?php _e( 'If the grid is disabled the number of rows and columns can be typed in the Insert Table dialog.', 'advance_your_editor' ); ?>
			</p>
		</div>
		<div>
			...
			<label for="table_tab_navigation"><?php _e( 'Jump to the next cell when pressing the tab key while editing a table', 'advance_your_editor' ); ?></label>
			<p>
				<?php _e( 'When disabled, pressing the tab key will jump outside the editor area.', 'advance_your_editor' ); ?>
			</p>
		</div>
		<div>
			...
			<label for="table_advtab"><?php _e( 'Show the advanced tabs in the table properties dialogs', 'advance_your_editor' ); ?></label>
			<p>
				<?php _e( 'The advanced tabs allow setting of inline CSS styles on the table, each row, and each cell. They have fields for easier setting of border, border color and background color styles.', 'advance_your_editor' ); ?>
			</p>
			<p>
				<span class="dashicons dashicons-warning"></span>
				<?php _e( 'To keep the table more responsive please use percentage values when setting widths.', 'advance_your_editor' ); ?>
			</p>
		</div>
	</div>
	<hr>
	<div>
		...
		$text = __( 'If you like the plugin, please donate N.G.O. We support %1$sDonate The Needy%2$s.', 'donate-needy' );

		$url = __( 'https://donatetheneedy.xyz/', 'donate-needy' );
		printf( $text, '<a href="' . esc_url( $url ) . '">', '</a>' );
	...
	<div class="advanced-options">
	<h3><?php _e( 'Administration', 'advance_your_editor' ); ?></h3>
	<div>
		<h4><?php _e( 'Settings import and export', 'advance_your_editor' ); ?></h4>
		<p>
			<?php _e( 'The settings are exported as a JSON encoded file.', 'advance_your_editor' ); ?>
			<?php _e( 'It is important that the exported file is not edited in any way.', 'advance_your_editor' ); ?>
		</p>
		<p>
			<?php wp_nonce_field( 'tadv-export-settings', 'tadv-export-settings-nonce', false ); ?>
			<input type="submit" class="button" name="tadv-export-settings" value="<?php _e( 'Export Settings', 'advance_your_editor' ); ?>" /> &nbsp;
			<input type="submit" class="button" name="tadv-import-settings" value="<?php _e( 'Import Settings', 'advance_your_editor' ); ?>" />
		</p>
	</div>
	<div>
		<h4><?php _e( 'Enable the editor enhancements for:', 'advance_your_editor' ); ?></h4>
		<p>
			...
			<label for="tadv_enable_1"><?php _e( 'The Classic Editor (Add New and Edit posts and pages)', 'advance_your_editor' ); ?></label>
		</p>
		<p>
			...
			<label for="tadv_enable_2"><?php _e( 'Other editors in wp-admin', 'advance_your_editor' ); ?></label>
		</p>
		<p>
			...
			<label for="tadv_enable_3"><?php _e( 'Editors on the front end of the site', 'advance_your_editor' ); ?></label>
		</p>
	</div>
	</div>
	<?php
...
<p class="tadv-submit">
	...
	<input class="button" type="submit" name="tadv-restore-defaults" value="<?php _e( 'Restore Default Settings', 'advance_your_editor' ); ?>" />
	<input class="button-primary button-large" type="submit" name="tadv-save" value="<?php _e( 'Save Changes', 'advance_your_editor' ); ?>" />
</p>
</form>

<div id="wp-adv-error-message" class="tadv-error">
<?php _e( 'The [Toolbar toggle] button shows or hides the second, third, and forth button rows. It will only work when it is in the first row and there are buttons in the second row.', 'advance_your_editor' ); ?>
</div>
</div><?php // .wrap.advance_your_editor end ?>
